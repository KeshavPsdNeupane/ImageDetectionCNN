...
private:
	...
	std::vector<float> CostForSigmoid(std::vector<float>result, int y);
	std::vector<float> BackPropError( const std::vector<std::vector<float>>& currentWeight, const std::vector<float>cost);
	std::vector<float> ActivationCost(const std::vector<float>& activelayer, const std::vector<float>& backPropagationCost);
	void UpdateWeightAndBias(std::vector<std::vector<float>>& weight, std::vector<float>& bias,
...
private:
	...
	float TotalCost(std::vector<float> result ,  int y);
};
